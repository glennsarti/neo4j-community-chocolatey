$PackageDefinition = @{
  "TemplateName" = "neo4j-community-v3.1";
  "PackageName" = "neo4j-community";
  "PackageVersion" = "3.3.7";
  "DownloadURL" = "http://neo4j.com/artifact.php?name=neo4j-community-3.3.7-windows.zip";
  "MD5Checksum" = "cd029e2131c7be55854c8194c5936243";
  "NeoZipSubdir" = "neo4j-community-3.3.7";
  "NeoServerApiJarSuffix" = "3.3.7";
  "PrivateJavaVersion" = "8.0.131.11";
  "PrivateJavaURIDir" = "d54c1d3a095b4ff2b6607d096fa80163/";
  "PrivateJreChecksumMD5" = "9458b62000daac0f48155323185f1c4c";
}
