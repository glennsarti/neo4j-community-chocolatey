$PackageDefinition = @{
  "TemplateName" = "neo4j-community-v3.1";
  "PackageName" = "neo4j-community";
  "PackageVersion" = "3.3.9";
  "DownloadURL" = "http://neo4j.com/artifact.php?name=neo4j-community-3.3.9-windows.zip";
  "MD5Checksum" = "8b2c7e03f0d385ded3205b4e6d604977";
  "NeoZipSubdir" = "neo4j-community-3.3.9";
  "NeoServerApiJarSuffix" = "3.3.9";
  "PrivateJavaVersion" = "8.0.131.11";
  "PrivateJavaURIDir" = "d54c1d3a095b4ff2b6607d096fa80163/";
  "PrivateJreChecksumMD5" = "9458b62000daac0f48155323185f1c4c";
}
