$PackageDefinition = @{
  "TemplateName" = "neo4j-community-v3.1";
  "PackageName" = "neo4j-community";
  "PackageVersion" = "3.4.5";
  "DownloadURL" = "http://neo4j.com/artifact.php?name=neo4j-community-3.4.5-windows.zip";
  "MD5Checksum" = "952a193f7b9da413737d85185c6c223b";
  "NeoZipSubdir" = "neo4j-community-3.4.5";
  "NeoServerApiJarSuffix" = "3.4.5";
  "PrivateJavaVersion" = "8.0.131.11";
  "PrivateJavaURIDir" = "d54c1d3a095b4ff2b6607d096fa80163/";
  "PrivateJreChecksumMD5" = "9458b62000daac0f48155323185f1c4c";
}
