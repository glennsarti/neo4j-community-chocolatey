$PackageDefinition = @{
  "TemplateName" = "neo4j-community-v3.1";
  "PackageName" = "neo4j-community";
  "PackageVersion" = "3.5.0";
  "DownloadURL" = "http://neo4j.com/artifact.php?name=neo4j-community-3.5.0-windows.zip";
  "MD5Checksum" = "4e5c9fd3135c08b0add9619072b3c54f";
  "NeoZipSubdir" = "neo4j-community-3.5.0";
  "NeoServerApiJarSuffix" = "3.5.0";
  "PrivateJavaVersion" = "8.0.131.11";
  "PrivateJavaURIDir" = "d54c1d3a095b4ff2b6607d096fa80163/";
  "PrivateJreChecksumMD5" = "9458b62000daac0f48155323185f1c4c";
}
