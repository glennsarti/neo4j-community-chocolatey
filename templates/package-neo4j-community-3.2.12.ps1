$PackageDefinition = @{
  "TemplateName" = "neo4j-community-v3.1";
  "PackageName" = "neo4j-community";
  "PackageVersion" = "3.2.12";
  "DownloadURL" = "http://neo4j.com/artifact.php?name=neo4j-community-3.2.12-windows.zip";
  "MD5Checksum" = "c8b34961c838c406ee107a47b3d2e55c";
  "NeoZipSubdir" = "neo4j-community-3.2.12";
  "NeoServerApiJarSuffix" = "3.2.12";
  "PrivateJavaVersion" = "8.0.131.11";
  "PrivateJavaURIDir" = "d54c1d3a095b4ff2b6607d096fa80163/";
  "PrivateJreChecksumMD5" = "9458b62000daac0f48155323185f1c4c";
}
